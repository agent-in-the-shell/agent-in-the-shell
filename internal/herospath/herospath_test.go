...
func TestDataPathAndConfigPath(t *testing.T) {
	...
	if got, want := DataDir("agentdesk"), filepath.Join("/home/u", ".local", "share", "heros", "agentdesk"); got != want {
		t.Fatalf("DataDir = %q, want %q", got, want)
	}
	if got, want := DataPath("agentdesk", "agentdesk.db"), filepath.Join("/home/u", ".local", "share", "heros", "agentdesk", "agentdesk.db"); got != want {
		t.Fatalf("DataPath = %q, want %q", got, want)
	}
	if got, want := ConfigPath("agenttask", "config.toml"), filepath.Join("/home/u", ".config", "heros", "agenttask", "config.toml"); got != want {
...
func TestResolveDataAndConfig_EnvOverrideWins(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", "/data")
	t.Setenv("AGENT_DESK_DB", "")
	if got, want := ResolveData("AGENT_DESK_DB", "agentdesk", "agentdesk.db"), filepath.Join("/data", "heros", "agentdesk", "agentdesk.db"); got != want {
		t.Fatalf("no override: %q, want %q", got, want)
	}
	t.Setenv("AGENT_DESK_DB", "/custom/desk.db")
	if got := ResolveData("AGENT_DESK_DB", "agentdesk", "agentdesk.db"); got != "/custom/desk.db" {
...
func TestResolveBin_PrefersDeployDir(t *testing.T) {
	...
	if got := ResolveBin("agent-pi"); got != "agent-pi" {
	...
	exe := filepath.Join(dir, "agent-pi")
	if err := os.WriteFile(exe, []byte("#!/bin/sh\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	if got := ResolveBin("agent-pi"); got != exe {
...
func TestResolveDataLegacy(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("XDG_DATA_HOME", tmp) // canonical = tmp/heros/agentdesk/agentdesk.db
	t.Setenv("AGENT_DESK_DB", "")
	canonical := DataPath("agentdesk", "agentdesk.db")
	legacy := filepath.Join(tmp, ".agentdesk", "agentdesk.db")

	// Fresh install: neither exists → canonical (new layout).
	if got := ResolveDataLegacy("AGENT_DESK_DB", "agentdesk", "agentdesk.db", legacy); got != canonical {
	...
	if got := ResolveDataLegacy("AGENT_DESK_DB", "agentdesk", "agentdesk.db", legacy); got != legacy {
	...
	t.Setenv("AGENT_DESK_DB", "/custom/d.db")
	if got := ResolveDataLegacy("AGENT_DESK_DB", "agentdesk", "agentdesk.db", legacy); got != "/custom/d.db" {
	...
	if got := ResolveDataLegacy("AGENT_DESK_DB", "agentdesk", "agentdesk.db", dirLegacy); got != DataPath("agentdesk", "agentdesk.db") {
